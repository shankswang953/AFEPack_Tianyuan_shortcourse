...
constexpr double kSourceStrength = 120.0;
double kSourceCenterX = 0.40;
double kSourceCenterY = 0.80;
double kSourceSigma = 0.075;
double kSensorCenterX = 0.78;
double kSensorCenterY = 0.22;
double kSensorSigma = 0.120;
constexpr int kAdaptationRounds = 4;
constexpr int kMarkingBudget[kAdaptationRounds] = {20, 40, 80, 160};
constexpr int kDwrAdaptationRounds = 1;
constexpr int kDwrMarkingBudget[kDwrAdaptationRounds] = {
    20};
constexpr double kDualColorGamma = 4.0;
constexpr int kReferenceModes = 80;
constexpr int kReferenceIntervals = 4096;

double heat_source(const double* point) {
  const double dx = point[0] - kSourceCenterX;
  const double dy = point[1] - kSourceCenterY;
  const double radius_squared = dx * dx + dy * dy;
  return kSourceStrength *
         std::exp(-radius_squared /
                  (2.0 * kSourceSigma * kSourceSigma));
}
...
double gaussian_basis_integral(double center,
  ...
  for (int i = 0; i <= intervals; ++i) {
    ...
    const double gaussian = std::exp(
        ...
        (2.0 * sigma * sigma));
    double basis = 0.0;
    if (dirichlet_neumann_basis) {
      basis = std::sqrt(2.0) *
              std::sin((mode + 0.5) * pi * coordinate);
    } else if (mode == 0) {
      basis = 1.0;
    } else {
      basis = std::sqrt(2.0) *
              std::cos(mode * pi * coordinate);
    }
...
double spectral_reference_sensor_temperature(int modes, int intervals) {
  ...
  for (int mode = 0; mode < modes; ++mode) {
    source_x[mode] = gaussian_basis_integral(
        kSourceCenterX, kSourceSigma, mode, false, intervals);
    source_y[mode] = gaussian_basis_integral(
        kSourceCenterY, kSourceSigma, mode, true, intervals);
    sensor_x[mode] = gaussian_basis_integral(
        kSensorCenterX, kSensorSigma, mode, false, intervals) /
        sensor_x_normalization;
    sensor_y[mode] = gaussian_basis_integral(
        kSensorCenterY, kSensorSigma, mode, true, intervals) /
  ...
  for (int x_mode = 0; x_mode < modes; ++x_mode) {
    for (int y_mode = 0; y_mode < modes; ++y_mode) {
      const double x_wave_number = x_mode * pi;
      const double y_wave_number = (y_mode + 0.5) * pi;
      const double eigenvalue =
          x_wave_number * x_wave_number + y_wave_number * y_wave_number;
      functional +=
          kSourceStrength * source_x[x_mode] * source_y[y_mode] *
...
void build_p1_space(
  ...
  // EasyMesh stores one marker per corner vertex.  At the bottom-right
  // corner the supplied unit-square mesh carries the right-side Neumann
  // marker, although that point also belongs to the cooled bottom boundary.
  // Give y=0 Dirichlet data priority for every nodal degree of freedom.
...
std::vector<bool> mark_largest(const std::vector<double>& indicator,
                               int marking_budget) {
  ...
  const int selected = std::min(
      marking_budget, static_cast<int>(indicator.size()));
...
void write_dual_magnitude_svg(
  ...
  const double source_x = svg_x(kSourceCenterX, bounds);
  const double source_y = svg_y(kSourceCenterY, bounds);
  const double source_radius =
      690.0 * kSourceSigma / (bounds.xmax - bounds.xmin);
  ...
  output << "<circle cx=\"" << source_x << "\" cy=\"" << source_y
         << "\" r=\"" << source_radius
         << "\" fill=\"none\" stroke=\"#d97706\" stroke-width=\"2.5\"/>\n"
         << "<text x=\"" << source_x + source_radius + 7.0
         << "\" y=\"" << source_y - 5.0
         << "\" font-family=\"sans-serif\" font-size=\"13\" "
         << "fill=\"#92400e\">heat source</text>\n"
...
struct DWRResult {
  ...
  std::vector<double> residual_dual_product;
...
DiscreteDualResult solve_discrete_dual(
  ...
  BoundaryFunction<double, kDimension> homogeneous_dual_bottom(
      BoundaryConditionInfo::DIRICHLET,
      kBottomBoundary,
      &cold_temperature);
  BoundaryConditionAdmin<double, kDimension> boundary_admin(fem_space);
  boundary_admin.add(homogeneous_dual_bottom);
  boundary_admin.apply(discrete_jacobian, dual, dual_right_hand_side);
...
DWRResult compute_dwr_indicator(
  ...
  BoundaryFunction<double, kDimension> cold_primal_bottom(
      BoundaryConditionInfo::DIRICHLET,
      kBottomBoundary,
      &cold_temperature);
  BoundaryConditionAdmin<double, kDimension> coarse_boundary_admin(
      coarse_space);
  coarse_boundary_admin.add(cold_primal_bottom);
  coarse_boundary_admin.apply(
      coarse_stiffness, coarse_primal, coarse_right_hand_side);
  ...
  BoundaryFunction<double, kDimension> cold_dual_bottom(
      BoundaryConditionInfo::DIRICHLET,
      kBottomBoundary,
      &cold_temperature);
  BoundaryConditionAdmin<double, kDimension> fine_boundary_admin(fine_space);
  fine_boundary_admin.add(cold_dual_bottom);
  fine_boundary_admin.apply(
      fine_jacobian, fine_dual, fine_dual_right_hand_side);
  ...
  result.residual_dual_product.assign(coarse_element_count, 0.0);
  ...
  // R_H(I_h^H U_h)(Z_H-I_h^H Pi_h Z_H).  Marking below uses its two-factor
  // Cauchy--Schwarz form: full primal residual norm times dual-correction RMS.
  ...
  for (int element_index = 0;
    ...
    result.primal_residual_norm[element_index] =
        std::sqrt(coarse_residual_eta_squared[element_index]);
    ...
    result.dual_solution_rms[element_index] =
        accumulated_volume[element_index] > 0.0
            ? std::sqrt(dual_solution_l2_squared[element_index] /
                        accumulated_volume[element_index])
            : 0.0;
    result.residual_dual_product[element_index] =
        result.primal_residual_norm[element_index] *
        result.dual_correction_rms[element_index];
...
void write_dwr_indicator_data(
  ...
  output << "# element centroid_x centroid_y signed_dwr abs_dwr "
            ...
            "dual_solution_rms residual_dual_product marked\n"
         << std::scientific << std::setprecision(12);
  for (unsigned int element_index = 0;
       element_index < mesh.n_geometry(kDimension); ++element_index) {
    const GeometryBM& geometry =
    ...
    output << element_index << ' ' << centroid[0] << ' ' << centroid[1]
           ...
           << ' ' << dwr.residual_dual_product[element_index]
           << ' ' << (marked[element_index] ? 1 : 0) << '\n';
  }
}
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 2 && argc != 8 && !uniform_reference_mode) {
    std::cerr
        << "usage: " << argv[0]
        << " ROOT_MESH [SOURCE_X SOURCE_Y SOURCE_SIGMA "
  ...
  if (argc == 8) {
    kSourceCenterX = std::stod(argv[2]);
    kSourceCenterY = std::stod(argv[3]);
    kSourceSigma = std::stod(argv[4]);
    kSensorCenterX = std::stod(argv[5]);
    kSensorCenterY = std::stod(argv[6]);
    kSensorSigma = std::stod(argv[7]);
    if (kSourceSigma <= 0.0 || kSensorSigma <= 0.0) {
      std::cerr << "source and sensor widths must be positive\n";
      return EXIT_FAILURE;
    }
  }
  ...
  try {
    ...
    for (int round = 1; round <= kAdaptationRounds; ++round) {
      ...
      const std::vector<bool> marked =
          mark_largest(marking_indicator, kMarkingBudget[round - 1]);
      ...
      write_indicator_svg(
          ...
          marked,
          indicator_stem + ".svg",
          "Cell residual with fixed-budget marks, round " +
              std::to_string(round));
      if (round == kAdaptationRounds) {
        write_indicator_data(current_mesh,
                             current.eta_squared,
                             current.cell_residual_squared,
                             current.interior_jump_squared,
      ...
      Indicator<kDimension> indicator(current_mesh);
      for (unsigned int i = 0; i < indicator.size(); ++i) {
        indicator[i] = marked[i] ? 1.0 : 0.0;
      }
    ...
    write_mesh_edges_svg(
        mesh_after,
        "mesh_after.svg",
        "Mesh after fixed-budget residual refinement");
    ...
    for (int round = 1; round <= kAdaptationRounds; ++round) {
      ...
      const std::vector<bool> dual_marked = mark_largest(
          dual_result.element_rms_magnitude,
          kMarkingBudget[round - 1]);
      ...
      write_dual_magnitude_svg(
          ...
          "Dual magnitude with fixed-budget marks, round " + round_string,
          true);
      ...
      Indicator<kDimension> dual_indicator(dual_mesh);
      for (unsigned int i = 0; i < dual_indicator.size(); ++i) {
        dual_indicator[i] = dual_marked[i] ? 1.0 : 0.0;
      }
      MeshAdaptor<kDimension> dual_adaptor(dual_irregular_mesh);
      dual_adaptor.convergenceOrder() = 0.0;
      dual_adaptor.refineStep() = 0;
      dual_adaptor.refineThreshold() = 1.0;
      dual_adaptor.tolerence() = 0.1;
      dual_adaptor.is_refine_only() = true;
      dual_adaptor.setIndicator(dual_indicator);
      dual_adaptor.adapt();
    ...
    // A third independent sequence uses a genuine DWR indicator.  The
    // primal remains P1 on the current mesh, while the fully discrete dual is
    // solved in P1 on one uniformly h-refined copy.  This keeps the adapted
    // primal DOF counts comparable with the two fixed-budget sequences above.
    ...
    for (int round = 1; round <= kDwrAdaptationRounds; ++round) {
      ...
      const std::vector<double>& dwr_marking_indicator =
          dwr.residual_dual_product;
      const std::vector<bool> dwr_marked = mark_largest(
          dwr_marking_indicator,
          kDwrMarkingBudget[round - 1]);
      ...
      write_indicator_svg(
          ...
          "Residual-dual product indicator, round " +
              round_string);
      ...
      if (round == kDwrAdaptationRounds) {
        write_indicator_svg(
            dwr_mesh,
            dwr_marking_indicator,
            dwr_marked,
            "dwr_indicator.svg",
            "DWR marking: primal residual times dual correction");
        write_indicator_svg(
            dwr_mesh,
            dwr.primal_residual_norm,
            dwr_marked,
            "dwr_primal_residual_factor.svg",
            "DWR factor 1: full primal residual norm");
        write_indicator_svg(
            dwr_mesh,
            dwr.dual_correction_rms,
            dwr_marked,
            "dwr_dual_correction_factor.svg",
            "DWR factor 2: enriched dual correction RMS");
        write_indicator_svg(
            dwr_mesh,
      ...
      Indicator<kDimension> dwr_mesh_indicator(dwr_mesh);
      for (unsigned int element_index = 0;
           element_index < dwr_mesh_indicator.size(); ++element_index) {
        dwr_mesh_indicator[element_index] =
            dwr_marked[element_index] ? 1.0 : 0.0;
      }
      MeshAdaptor<kDimension> dwr_adaptor(dwr_irregular_mesh);
      dwr_adaptor.convergenceOrder() = 0.0;
      dwr_adaptor.refineStep() = 0;
      dwr_adaptor.refineThreshold() = 1.0;
      dwr_adaptor.tolerence() = 0.1;
      dwr_adaptor.is_refine_only() = true;
      dwr_adaptor.setIndicator(dwr_mesh_indicator);
      dwr_adaptor.adapt();
    ...
    dwr_history << kDwrAdaptationRounds << ' '
    ...
    std::cout << std::setprecision(8)
              ...
              << "  source center       : (" << kSourceCenterX << ", "
              << kSourceCenterY << ")\n"
              << "  source sigma        : " << kSourceSigma << '\n'
    ...
    for (int round = 0; round < kAdaptationRounds; ++round) {
      std::cout << (round == 0 ? "" : ", ") << kMarkingBudget[round];
    }
    std::cout << " elements\n"
    ...
    std::cout << "Independent discrete dual\n"
              << "  marking rule        : same fixed budgets as residual\n"
    ...
    std::cout << "DWR refinement\n"
              ...
              << "  marking rule        : residual norm times dual correction RMS\n"
              << "  marking budgets     : ";
    for (int round = 0; round < kDwrAdaptationRounds; ++round) {
      std::cout << (round == 0 ? "" : ", ")
                << kDwrMarkingBudget[round];
    }
    std::cout << " elements\n";
    for (const DWRAdaptationReport& report : dwr_reports) {
      const double corrected_functional =
    ...
    std::cerr << "poisson_residual_adaptive: " << error.what() << '\n';
    return EXIT_FAILURE;
  }
