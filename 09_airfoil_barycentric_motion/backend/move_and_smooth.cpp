...
std::vector<std::vector<int>> build_vertex_incidence(const RegularMesh<2>& mesh)
{
  std::vector<std::vector<int>> incidence(
      static_cast<std::size_t>(mesh.n_point()));
  for (int element = 0; element < mesh.n_geometry(2); ++element) {
    const Geometry& geometry = mesh.geometry(2, element);
    for (int vertex : geometry.vertex()) {
      incidence[static_cast<std::size_t>(vertex)].push_back(element);
    }
  }
  return incidence;
}

double oriented_shape_quality(
    const RegularMesh<2>& mesh,
    int element,
    const std::vector<double>& reference_orientation)
{
  ...
  const double bc2 = std::pow(b[0] - c[0], 2) + std::pow(b[1] - c[1], 2);
  const double ca2 = std::pow(c[0] - a[0], 2) + std::pow(c[1] - a[1], 2);
  return 4.0 * std::sqrt(3.0) * area / (ab2 + bc2 + ca2);
}

double local_minimum_quality(
    const RegularMesh<2>& mesh,
    int vertex,
    ...
    const std::vector<double>& reference_orientation)
{
  ...
  for (int element : incidence[static_cast<std::size_t>(vertex)]) {
    minimum = std::min(
        minimum,
        oriented_shape_quality(mesh, element, reference_orientation));
  }
  return minimum;
}

bool try_quality_improving_move(
    RegularMesh<2>& mesh,
    int vertex,
    const Point2& target,
    const std::vector<std::vector<int>>& incidence,
    const std::vector<double>& reference_orientation)
{
  const Point2 original = {mesh.point(vertex)[0], mesh.point(vertex)[1]};
  Point2 best = original;
  double best_quality = local_minimum_quality(
      mesh, vertex, incidence, reference_orientation);
  const double fractions[] = {
      1.0, 0.75, 0.50, 0.35, 0.25,
      0.15, 0.10, 0.05, 0.025, 0.0125};

  for (double fraction : fractions) {
    mesh.point(vertex)[0] =
        original.x + fraction * (target.x - original.x);
    mesh.point(vertex)[1] =
        original.y + fraction * (target.y - original.y);
    const double candidate_quality = local_minimum_quality(
        mesh, vertex, incidence, reference_orientation);
    if (candidate_quality > best_quality + 1.0e-12) {
      best_quality = candidate_quality;
      best = {mesh.point(vertex)[0], mesh.point(vertex)[1]};
    }
  }

  mesh.point(vertex)[0] = best.x;
  mesh.point(vertex)[1] = best.y;
  return std::hypot(best.x - original.x, best.y - original.y) > 1.0e-15;
}
...
std::vector<VertexTarget> equilateral_airfoil_targets(
    const RegularMesh<2>& mesh)
{
  std::vector<Point2> target_sum(
      static_cast<std::size_t>(mesh.n_point()));
  std::vector<int> target_count(
      static_cast<std::size_t>(mesh.n_point()), 0);

  for (int element = 0; element < mesh.n_geometry(2); ++element) {
    const Geometry& geometry = mesh.geometry(2, element);
    const auto& vertices = geometry.vertex();
    std::vector<int> boundary_vertices;
    int interior_vertex = -1;
    for (int vertex : vertices) {
      if (mesh.boundaryMark(0, vertex) == 3) {
        boundary_vertices.push_back(vertex);
      } else if (mesh.boundaryMark(0, vertex) == 0) {
        interior_vertex = vertex;
      }
    }
    if (boundary_vertices.size() != 2 || interior_vertex < 0) {
      continue;
    }

    const AFEPack::Point<2>& a = mesh.point(boundary_vertices[0]);
    const AFEPack::Point<2>& b = mesh.point(boundary_vertices[1]);
    const AFEPack::Point<2>& current = mesh.point(interior_vertex);
    const double edge_x = b[0] - a[0];
    const double edge_y = b[1] - a[1];
    const double edge_length = std::hypot(edge_x, edge_y);
    if (!(edge_length > 0.0)) {
      continue;
    }
    const double midpoint_x = 0.5 * (a[0] + b[0]);
    const double midpoint_y = 0.5 * (a[1] + b[1]);
    const double normal_x = -edge_y / edge_length;
    const double normal_y = edge_x / edge_length;
    const double side =
        (current[0] - midpoint_x) * normal_x
            + (current[1] - midpoint_y) * normal_y
        >= 0.0
        ? 1.0
        : -1.0;
    const double height = 0.5 * std::sqrt(3.0) * edge_length;
    target_sum[static_cast<std::size_t>(interior_vertex)].x +=
        midpoint_x + side * height * normal_x;
    target_sum[static_cast<std::size_t>(interior_vertex)].y +=
        midpoint_y + side * height * normal_y;
    ++target_count[static_cast<std::size_t>(interior_vertex)];
  }

  std::vector<VertexTarget> targets;
  for (int vertex = 0; vertex < mesh.n_point(); ++vertex) {
    const int count = target_count[static_cast<std::size_t>(vertex)];
    if (count == 0) {
      continue;
    }
    targets.push_back(
        {
            vertex,
            {
                target_sum[static_cast<std::size_t>(vertex)].x / count,
                target_sum[static_cast<std::size_t>(vertex)].y / count,
            },
        });
  }
  return targets;
...
{
  if (iterations < 0 || !(relaxation > 0.0 && relaxation <= 1.0)) {
    throw std::runtime_error("invalid boundary-quality smoothing parameters");
  }

  const std::vector<std::vector<int>> adjacency =
      build_vertex_adjacency(mesh);
  const std::vector<std::vector<int>> incidence =
      build_vertex_incidence(mesh);
  const std::vector<VertexTarget> boundary_targets =
      equilateral_airfoil_targets(mesh);

  for (int iteration = 0; iteration < iterations; ++iteration) {
    int accepted_moves = 0;

    // First improve the triangles attached directly to the airfoil.  Moving
    // all the way to the equilateral target may damage the next layer, so a
    // local line search accepts only a strict improvement of every affected
    // vertex patch.
    for (const VertexTarget& target : boundary_targets) {
      accepted_moves += try_quality_improving_move(
          mesh,
          target.vertex,
          target.point,
          incidence,
    ...
    // Then propagate the correction into the remaining mesh with a smart
    // Laplacian sweep.  A centroid proposal is retained only when the
    // minimum quality of the incident triangles increases.
    for (int vertex = 0; vertex < mesh.n_point(); ++vertex) {
      if (mesh.boundaryMark(0, vertex) != 0
          || adjacency[static_cast<std::size_t>(vertex)].empty()) {
        continue;
      ...
      Point2 average;
      for (int neighbor : adjacency[static_cast<std::size_t>(vertex)]) {
        average.x += mesh.point(neighbor)[0];
        average.y += mesh.point(neighbor)[1];
      }
      const double count =
          static_cast<double>(adjacency[static_cast<std::size_t>(vertex)].size());
      average.x /= count;
      average.y /= count;
      const Point2 relaxed_target = {
          mesh.point(vertex)[0]
              + relaxation * (average.x - mesh.point(vertex)[0]),
          mesh.point(vertex)[1]
              + relaxation * (average.y - mesh.point(vertex)[1]),
      };
      accepted_moves += try_quality_improving_move(
          mesh,
          vertex,
  ...
  try {
    if (argc < 5 || argc > 8) {
      std::cerr
          ...
          << " [laplacian_iterations=0] [relaxation=0.45]"
          << " [boundary_quality_iterations=20]\n";
      return 1;
    }
    ...
    const int iterations = argc > 5 ? std::stoi(argv[5]) : 0;
    const double relaxation = argc > 6 ? std::stod(argv[6]) : 0.45;
    const int boundary_quality_iterations =
        argc > 7 ? std::stoi(argv[7]) : 20;
    ...
    const Quality laplacian_quality =
        measure_quality(mesh, reference_orientation);
    write_stage(mesh, output_dir, "mesh_laplacian");

    boundary_quality_smooth(
        mesh,
        boundary_quality_iterations,
        relaxation,
        reference_orientation);
    ...
    summary << "laplacian," << laplacian_quality.minimum_area << ","
            << laplacian_quality.minimum_shape_quality << ","
            << laplacian_quality.inverted_elements << "\n";
    ...
    print_quality("after Laplacian smooth", laplacian_quality);
    print_quality("after boundary quality", smoothed_quality);
