...
IMAGE_CONVERT_BIN=/path/to/convert
```
...
| Python helper/visualization | figures, animations, and the Python-based examples 08--11 | `PYTHON_BIN`, `PLOT_PYTHON`, `ENABLE_PYTHON_PLOTS` |
| PNG conversion | raster figures generated by the C++ shape example | `IMAGE_CONVERT_BIN` |
...
| airfoil design-space sampling (`11`) | `scipy` |
...
Example `07` uses ImageMagick's `convert` command to publish PNG figures. On
macOS with MacPorts, install it with `sudo port install ImageMagick`; otherwise
set `IMAGE_CONVERT_BIN` to the corresponding executable on your system.
...
| `IMAGE_CONVERT_BIN` | `/opt/local/bin/convert`, then `PATH` | ImageMagick is installed elsewhere |
...
12. `11_airfoil_design_space`: sample a five-dimensional engineering airfoil
    parameter space, reject inadmissible curves, select a dispersed design set,
    and generate an EasyMesh grid for every selected geometry.

Examples `00`--`05` use the EasyMesh input
`common/mesh/unit_square/D.[nse]`. Examples `06`--`11` generate their own
...
| `11_airfoil_design_space` | `./run.sh --candidates 2048 --selected 6 --seed 2026` | filter a wide five-parameter sample and mesh six dispersed airfoils |
